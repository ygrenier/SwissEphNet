    ...
    partial class SwissEphTest
    {
        ...
        public void Test_swe_set_topo()
        {
            using (var swe = new SwissEph())
            {
                ...
                Assert.AreEqual(142.781800327557, xin2[0], 0.0000000000001);
                Assert.AreEqual(0.0000571321650547235, xin2[1], 0.0000000000001);
                Assert.AreEqual(1.01269578080906, xin2[2], 0.0000000000001);

                swe.swe_calc(tjd, SwissEph.SE_SUN, 33024, xin3, ref serr);
                Assert.AreEqual(142.781800327557, xin3[0], 0.0000000000001);
