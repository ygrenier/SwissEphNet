...
//#else
        const double DSUN = (1392000000.0 / AUNIT);
//#endif
        const double DMOON = (3476300.0 / AUNIT);
        const double DEARTH = (6378140.0 * 2 / AUNIT);
...
//static int32 calc_planet_star(double tjd_et, int32 ipl, char *starname, int32 iflag, double *x, char *serr);
        ...
        public Int32 swe_sol_eclipse_where(
            ...
            ifl &= SEFLG_EPHMASK;
            if ((retflag = eclipse_where(tjd_ut, SE_SUN, null, ifl, geopos, dcore, ref serr)) < 0)
                return retflag;
            if ((retflag2 = eclipse_how(tjd_ut, SE_SUN, null, ifl, geopos[0], geopos[1], 0, attr, ref serr)) == ERR)
                return retflag2;
            attr[3] = dcore[0];
            return retflag;
        }
        ...
        public Int32 swe_lun_occult_where(
            ...
            if ((retflag = eclipse_where(tjd_ut, ipl, starname, ifl, geopos, dcore, ref serr)) < 0)
                return retflag;
            if ((retflag2 = eclipse_how(tjd_ut, ipl, starname, ifl, geopos[0], geopos[1], 0, attr, ref serr)) == ERR)
                return retflag2;
            attr[3] = dcore[0];
            return retflag;
        }
        ...
        Int32 eclipse_where(double tjd_ut, Int32 ipl, string starname, Int32 ifl, double[] geopos, double[] dcore,
            ...
            if ((retc = calc_planet_star(tjd, ipl, starname, iflag2, ls, ref serr)) == ERR)
                return retc;
            /* save sun position */
            for (i = 0; i <= 2; i++)
                rst[i] = rs[i];
        ...
        iter_where:
            ...
            if (0 == (retc & SE_ECL_PARTIAL) && !no_eclipse) {
                if (dcore[0] > 0) {
                    /*printf("annular\n");*/
                    retc |= SE_ECL_ANNULAR;
                } else {
        ...
        Int32 swe_sol_eclipse_how(
            ...
            ifl &= SEFLG_EPHMASK;
            if ((retflag = eclipse_how(tjd_ut, SE_SUN, null, ifl, geopos[0], geopos[1], geopos[2], attr, ref serr)) == ERR)
                return retflag;
            if ((retflag2 = eclipse_where(tjd_ut, SE_SUN, null, ifl, geopos2, dcore, ref serr)) == ERR)
                return retflag2;
            if (retflag!=0)
...
//static int USE_AZ_NAV =0;
  ...
  swe_cotrans(xh, xh, 90 - geolat); /* azimuth from east, counterclock, via north */
            ...
            rsun = Math.Asin(drad / ls[2]) * RADTODEG;
            rsplusrm = rsun + rmoon;
            ...
            dctr = Math.Acos(swi_dot_prod_unit(x1, x2)) * RADTODEG;
            /* 
             * phase 
             */
            if (dctr < rsminusrm)
             ...
             */
            lsun = Math.Asin(rsun / 2 * DEGTORAD) * 2;
  ...
  attr[4] = swe_degnorm(90 - xh[0]); /* azimuth, from north, clockwise, via east */
            ...
            attr[5] = xh[1]; /* height */
            attr[6] = xh[2]; /* height */
            if (ipl == SE_SUN && String.IsNullOrEmpty(starname)) {
        ...
        next_try:
            ...
            for (dt = dtstart;
                ...
                for (i = 0, t = tjd - dt; i <= 2; i++, t += dt) {
                    ...
                    dc[i] = Math.Acos(swi_dot_prod_unit(xa, xb)) * RADTODEG;
                    rmoon = Math.Asin(RMOON / lm[2]) * RADTODEG;
                    rsun = Math.Asin(RSUN / ls[2]) * RADTODEG;
                    dc[i] -= (rmoon + rsun);
                }
            ...
            if ((retflag = eclipse_where(tjd, SE_SUN, null, ifl, geopos, dcore, ref serr)) == ERR)
                return retflag;
            retflag2 = retflag;
            ...
            if ((retflag2 = eclipse_how(tjd, SE_SUN, null, ifl, geopos[0], geopos[1], 0, attr, ref serr)) == ERR)
                return retflag2;
            if (retflag2 == 0) {
                K += direction;
                goto next_try;
            ...
            if (0 == (ifltype & SE_ECL_NONCENTRAL) && (retflag & SE_ECL_NONCENTRAL) != 0) {
                K += direction;
                goto next_try;
            }
            /* central eclipse is wanted: */
            if (0 == (ifltype & SE_ECL_CENTRAL) && (retflag & SE_ECL_CENTRAL) != 0) {
                K += direction;
                goto next_try;
            }
            /* non annular eclipse is wanted: */
            if (0 == (ifltype & SE_ECL_ANNULAR) && (retflag & SE_ECL_ANNULAR) != 0) {
                K += direction;
                goto next_try;
            }
            /* non partial eclipse is wanted: */
            if (0 == (ifltype & SE_ECL_PARTIAL) && (retflag & SE_ECL_PARTIAL) != 0) {
                K += direction;
                goto next_try;
            }
            /* annular-total eclipse will be discovered later */
            if (0 == (ifltype & (SE_ECL_TOTAL | SE_ECL_ANNULAR_TOTAL)) && (retflag & SE_ECL_TOTAL) != 0) {
                K += direction;
                goto next_try;
            }
            if (dont_times)
            ...
            for (n = 0; n <= o; n++) {
                ...
                for (i = 0, t = tjd - dta; i <= 2; i += 1, t += dta) {
                    if ((retflag2 = eclipse_where(t, SE_SUN, null, ifl, geopos, dcore, ref serr)) == ERR)
                        return retflag2;
                    if (n == 0)
            ...
            if ((retflag & SE_ECL_TOTAL) != 0) {
                if ((retflag2 = eclipse_where(tret[0], SE_SUN, null, ifl, geopos, dcore, ref serr)) == ERR)
                    return retflag2;
                dc[0] = dcore[0];
                if ((retflag2 = eclipse_where(tret[4], SE_SUN, null, ifl, geopos, dcore, ref serr)) == ERR)
                    return retflag2;
                dc[1] = dcore[0];
                if ((retflag2 = eclipse_where(tret[5], SE_SUN, null, ifl, geopos, dcore, ref serr)) == ERR)
                    return retflag2;
                dc[2] = dcore[0];
            ...
            if (0 == (ifltype & SE_ECL_TOTAL) && (retflag & SE_ECL_TOTAL) != 0) {
                K += direction;
                goto next_try;
            }
            /* if annular_total eclipse is given but not wanted: */
            if (0 == (ifltype & SE_ECL_ANNULAR_TOTAL) && (retflag & SE_ECL_ANNULAR_TOTAL) != 0) {
                K += direction;
                goto next_try;
            }
            /*
        ...
        next_try:
            ...
            tjd -= swe_deltat(tjd);
            tjds = tjd;
            if ((retflag = eclipse_where(tjd, ipl, starname, ifl, geopos, dcore, ref serr)) == ERR)
                return retflag;
            retflag2 = retflag;
            /* in extreme cases _where() returns no eclipse, where there is
             * actually a very small one, therefore call _how() with the
            ...
            for (n = 0; n <= o; n++) {
                ...
                for (i = 0, t = tjd - dta; i <= 2; i += 1, t += dta) {
                    if ((retflag2 = eclipse_where(t, ipl, starname, ifl, geopos, dcore, ref serr)) == ERR)
                        return retflag2;
                    if (n == 0)
                        dc[i] = dcore[4] / 2 + de / dcore[5] - dcore[2];
                    else if (n == 1)
             ...
             */
            if ((retflag & SE_ECL_TOTAL) != 0) {
                if ((retflag2 = eclipse_where(tret[0], ipl, starname, ifl, geopos, dcore, ref serr)) == ERR)
                    return retflag2;
                dc[0] = dcore[0];
                if ((retflag2 = eclipse_where(tret[4], ipl, starname, ifl, geopos, dcore, ref serr)) == ERR)
                    return retflag2;
                dc[1] = dcore[0];
                if ((retflag2 = eclipse_where(tret[5], ipl, starname, ifl, geopos, dcore, ref serr)) == ERR)
                    return retflag2;
                dc[2] = dcore[0];
                /* the maximum is always total, and there is either one or
                 * to times before and after, when the core shadow becomes
        ...
        public Int32 swe_sol_eclipse_when_loc(double tjd_start, Int32 ifl,
             ...
             * diameter of core shadow
             */
            if ((retflag2 = eclipse_where(tret[0], SE_SUN, null, ifl, geopos2, dcore, ref serr)) == ERR)
                return retflag2;
        ...
        public Int32 swe_lun_occult_when_loc(double tjd_start, Int32 ipl, string starname, Int32 ifl,
             ...
             * diameter of core shadow
             */
            if ((retflag2 = eclipse_where(tret[0], ipl, starname, ifl, geopos2, dcore, ref serr)) == ERR)
                return retflag2;
            retflag |= (retflag2 & SE_ECL_NONCENTRAL);
            attr[3] = dcore[0];
            return retflag;
        ...
        next_try:
            ...
            for (dt = dtstart;
                ...
                for (i = 0, t = tjd - dt; i <= 2; i++, t += dt) {
                    ...
                    dc[i] = Math.Acos(swi_dot_prod_unit(x1, x2)) * RADTODEG;
                }
                find_maximum(dc[0], dc[1], dc[2], dt, out dtint, out dctr);
                tjd += dtint + dt;
            }
            ...
            if (dctr > Math.Abs(rsminusrm))  /* partial, no 2nd and 3rd contact */
                ...
                for (i = 0, t = tjd - twomin; i <= 2; i += 2, t = tjd + twomin) {
                    ...
                    rsun = Math.Asin(RSUN / ds) * RADTODEG;
                    rsminusrm = rsun - rmoon;
                    for (k = 0; k < 3; k++) {
                        x1[k] = xs[k] / ds /*ls[2]*/;
                        x2[k] = xm[k] / dm /*lm[2]*/;
            ...
            //#endif
            if (swe_rise_trans(tret[1] - 0.1, SE_SUN, null, iflag, SE_CALC_RISE | SE_BIT_DISC_BOTTOM, geopos, 0, 0, ref tjdr, ref serr) == ERR)
                return ERR;
            if (swe_rise_trans(tret[1] - 0.1, SE_SUN, null, iflag, SE_CALC_SET | SE_BIT_DISC_BOTTOM, geopos, 0, 0, ref tjds, ref serr) == ERR)
                return ERR;
            if (tjdr > tret[1] && tjdr < tret[4]) {
                tret[5] = tjdr;
                if (0 == (retflag & SE_ECL_MAX_VISIBLE)) {
                    tret[0] = tjdr;
                    if ((retc = eclipse_how(tret[5], SE_SUN, null, ifl, geopos[0], geopos[1], geopos[2], attr, ref serr)) == ERR)
                        return ERR;
            ...
            if (tjds > tret[1] && tjds < tret[4]) {
                ...
                if (0 == (retflag & SE_ECL_MAX_VISIBLE)) {
                    tret[0] = tjds;
                    if ((retc = eclipse_how(tret[6], SE_SUN, null, ifl, geopos[0], geopos[1], geopos[2], attr, ref serr)) == ERR)
                        return ERR;
                    retflag &= ~(SE_ECL_TOTAL | SE_ECL_ANNULAR | SE_ECL_PARTIAL);
                    retflag |= (retc & (SE_ECL_TOTAL | SE_ECL_ANNULAR | SE_ECL_PARTIAL));
                }
        ...
        next_try:
            ...
            if (calc_planet_star(tjd, ipl, starname, iflagcart, xs, ref serr) == ERR)
                return ERR;
            ...
            //#endif
            if (swe_rise_trans(tret[1] - 0.1, ipl, starname, iflag, SE_CALC_RISE | SE_BIT_DISC_BOTTOM, geopos, 0, 0, ref tjdr, ref serr) == ERR)
                return ERR;
            if (swe_rise_trans(tret[1] - 0.1, ipl, starname, iflag, SE_CALC_SET | SE_BIT_DISC_BOTTOM, geopos, 0, 0, ref tjds, ref serr) == ERR)
                return ERR;
            if (tjdr > tret[1] && tjdr < tret[4])
                tret[5] = tjdr;
            if (tjds > tret[1] && tjds < tret[4])
                tret[6] = tjds;
            if (swe_rise_trans(tret[2], SE_SUN, null, iflag, SE_CALC_RISE, geopos, 0, 0, ref tjdr, ref serr) == ERR)
                return ERR;
            if (swe_rise_trans(tret[2], SE_SUN, null, iflag, SE_CALC_SET, geopos, 0, 0, ref tjds, ref serr) == ERR)
                return ERR;
            if (tjds < tjdr)
                retflag |= SE_ECL_OCC_BEG_DAYLIGHT;
            if (swe_rise_trans(tret[3], SE_SUN, null, iflag, SE_CALC_RISE, geopos, 0, 0, ref tjdr, ref serr) == ERR)
                return ERR;
            if (swe_rise_trans(tret[3], SE_SUN, null, iflag, SE_CALC_SET, geopos, 0, 0, ref tjds, ref serr) == ERR)
                return ERR;
            if (tjds < tjdr)
        ...
        */
        ...
        next_try:
            ...
            F *= DEGTORAD;
            Om *= DEGTORAD;
            F1 = F - 0.02665 * Math.Sin(Om) * DEGTORAD;
            A1 *= DEGTORAD;
            tjd = tjd - 0.4075 * Math.Sin(Mm)
                      + 0.1721 * E * Math.Sin(M)
            ...
            for (j = 0, dt = dtstart;
                ...
                for (i = 0, t = tjd - dt; i <= 2; i++, t += dt) {
                    ...
                    dc[i] = Math.Acos(swi_dot_prod_unit(xa, xb)) * RADTODEG;
                    rearth = Math.Asin(REARTH / dm) * RADTODEG;
                    rsun = Math.Asin(RSUN / ds) * RADTODEG;
                    dc[i] -= (rearth + rsun);
                }
                find_maximum(dc[0], dc[1], dc[2], dt, out dtint, out dctr);
            ...
            if ((retflag = swe_lun_eclipse_how(tjd, ifl, null, attr, ref serr)) == ERR)
                return retflag;
            if (retflag == 0) {
                K += direction;
                goto next_try;
            ...
            if (0 == (ifltype & SE_ECL_PENUMBRAL) && (retflag & SE_ECL_PENUMBRAL) != 0) {
                K += direction;
                goto next_try;
            }
            /* non partial eclipse is wanted: */
            ...
            for (n = 0; n <= o; n++) {
                ...
                //#if 1
                for (i = 0, t = tjd - dta; i <= 2; i += 1, t += dta) {
                    if ((retflag2 = lun_eclipse_how(t, ifl, attr, dcore, ref serr)) == ERR)
                        return retflag2;
                    if (n == 0)
        ...
        public Int32 swe_lun_eclipse_when_loc(double tjd_start, Int32 ifl,
            ...
            ifl &= ~(SEFLG_JPLHOR | SEFLG_JPLHOR_APPROX);
        next_lun_ecl:
            if ((retflag = swe_lun_eclipse_when(tjd_start, ifl, 0, tret, backward, ref serr)) == ERR) {
                return ERR;
            }
            /*  
             * visibility of eclipse phases 
            ...
            for (i = 7; i >= 0; i--) {
                ...
                if ((retflag2 = swe_lun_eclipse_how(tret[i], ifl, geopos, attr, ref serr)) == ERR)
                    return ERR;
            ...
            if (swe_rise_trans(tret[6] - 0.1, SE_MOON, null, ifl, SE_CALC_RISE | SE_BIT_DISC_BOTTOM, geopos, 0, 0, ref tjdr, ref serr) == ERR)
                return ERR;
            if (swe_rise_trans(tret[6] - 0.1, SE_MOON, null, ifl, SE_CALC_SET | SE_BIT_DISC_BOTTOM, geopos, 0, 0, ref tjds, ref serr) == ERR)
                return ERR;
            tjd_max = tret[0];
            ...
            tret[0] = tjd_max;
            if ((retflag2 = swe_lun_eclipse_how(tjd_max, ifl, geopos, attr, ref serr)) == ERR)
                return ERR;
        ...
        const int NMAG_ELEM = (SE_VESTA + 1);
        ...
        public Int32 swe_pheno(double tjd, Int32 ipl, Int32 iflag, double[] attr, ref string serr) {
            ...
            if (ipl != SE_SUN && ipl != SE_EARTH &&
              ...
              ipl != SE_MEAN_APOG && ipl != SE_OSCU_APOG) {
                 ...
                 * phase angle
                 */
                attr[0] = Math.Acos(swi_dot_prod_unit(xx, xx2)) * RADTODEG;
                /*
                 * phase
                 */
            ...
            if (ipl > SE_AST_OFFSET || (ipl < NMAG_ELEM && mag_elem[ipl, 0] < 99)) {
                if (ipl == SE_SUN) {
                    ...
                    fac = attr[3] / (Math.Asin(pla_diam[SE_SUN] / 2.0 / AUNIT) * 2 * RADTODEG);
                    fac *= fac;
                    ...
                    attr[4] = -21.62 + 5 * Math.Log10(lbr[2] * AUNIT / EARTH_RADIUS) / Math.Log10(10) + 0.026 * Math.Abs(attr[0]) + 0.000000004 * Math.Pow(attr[0], 4);
                    //#if 0
                    //      /* ratio apparent diameter : average diameter */
                    ...
                    inx = (28.075216 - 0.012998 * T + 0.000004 * T * T) * DEGTORAD;
                    om = (169.508470 + 1.394681 * T + 0.000412 * T * T) * DEGTORAD;
                    sinB = Math.Abs(Math.Sin(inx) * Math.Cos(lbr[1] * DEGTORAD)
                                  * Math.Sin(lbr[0] * DEGTORAD - om)
                                  - Math.Cos(inx) * Math.Sin(lbr[1] * DEGTORAD));
                    u1 = Math.Atan2(Math.Sin(inx) * Math.Tan(lbr2[1] * DEGTORAD)
                                           + Math.Cos(inx) * Math.Sin(lbr2[0] * DEGTORAD - om),
                                      Math.Cos(lbr2[0] * DEGTORAD - om)) * RADTODEG;
                    u2 = Math.Atan2(Math.Sin(inx) * Math.Tan(lbr[1] * DEGTORAD)
                                           + Math.Cos(inx) * Math.Sin(lbr[0] * DEGTORAD - om),
                                      Math.Cos(lbr[0] * DEGTORAD - om)) * RADTODEG;
                    ...
                    ph1 = Math.Pow(EULER, -3.33 * Math.Pow(Math.Tan(attr[0] * DEGTORAD / 2), 0.63));
                    ph2 = Math.Pow(EULER, -1.87 * Math.Pow(Math.Tan(attr[0] * DEGTORAD / 2), 1.22));
        ...
        Int32 swe_pheno_ut(double tjd_ut, Int32 ipl, Int32 iflag, double[] attr, ref string serr) {
            return swe_pheno(tjd_ut + swe_deltat(tjd_ut), ipl, iflag, attr, ref serr);
        }
        ...
        public Int32 swe_rise_trans_true_hor(
            ...
            if (0 == (rsmi & (SE_CALC_RISE | SE_CALC_SET)))
                rsmi |= SE_CALC_RISE;
            /* twilight calculation */
            if (ipl == SE_SUN && (rsmi & (SE_BIT_CIVIL_TWILIGHT | SE_BIT_NAUTIC_TWILIGHT | SE_BIT_ASTRO_TWILIGHT)) != 0) {
             ...
             */
            if (do_fixstar) {
                if (swe_fixstar(starname, tjd_et, iflag, xc, ref serr) == ERR)
                    return ERR;
            }
            for (ii = 0, t = tjd_ut - twohrs; ii <= jmax; ii++, t += twohrs) {
                ...
                rdi = Math.Asin(dd / 2 / AUNIT / curdist) * RADTODEG;
            ...
            for (ii = 1; ii <= jmax; ii++) {
                if (h[ii - 1] * h[ii] >= 0)
                    continue;
                if (h[ii - 1] < h[ii] && 0 == (rsmi & SE_CALC_RISE))
                    continue;
                if (h[ii - 1] > h[ii] && 0 == (rsmi & SE_CALC_SET))
                    continue;
                dc[0] = h[ii - 1];
                dc[1] = h[ii];
                t2[0] = tc[ii - 1];
        ...
        public Int32 swe_nod_aps(double tjd_et, Int32 ipl, Int32 iflag,
            ...
            bool do_aberr = 0 == (iflag & (SEFLG_TRUEPOS | SEFLG_NOABERR));
            bool do_defl = 0 == (iflag & SEFLG_TRUEPOS) && 0 == (iflag & SEFLG_NOGDEFL);
            bool do_focal_point = (method & SE_NODBIT_FOPOINT) != 0;
            ...
            if ((method == 0 || (method & SE_NODBIT_MEAN) != 0) &&
                  ((ipl >= SE_SUN && ipl <= SE_NEPTUNE) || ipl == SE_EARTH)) {
                if (ipl == SE_MOON) {
                    double[] xdummy = new double[] { xna[0], xna[3], xpe[0], xpe[3] };
                    swi_mean_lunar_elements(tjd_et, ref xdummy[0], ref xdummy[1], ref xdummy[2], ref xdummy[3]);
                ...
                if (ipli != SE_MOON) {
                    if ((method & SE_NODBIT_OSCU_BAR) != 0 && x[2] > 6) {
                ...
                for (i = istart; i <= iend; i++) {
                    ...
                    sinnode = xn[i][1] / rxy;
                    /* inclination */
                    swi_cross_prod(xpos[i], xpos[i].GetPointer( 3), xnorm);
            ...
            if ((iflag & SEFLG_TOPOCTR) != 0) {
                ...
                if (swi_get_observer(tjd_et, iflag, false, xobs, ref serr) != OK)
                    return ERR;
                /*for (i = 0; i <= 5; i++)
            ...
            if ((iflag & (SEFLG_HELCTR | SEFLG_BARYCTR)) != B1950) {
                if ((iflag & SEFLG_HELCTR)!=0 && 0 == (iflag & SEFLG_MOSEPH))
                    for (i = 0; i <= 5; i++)
                        xobs[i] = xsun[i];
            } else if (ipl == SE_SUN && 0 == (iflag & SEFLG_MOSEPH)) {
                for (i = 0; i <= 5; i++)
                    xobs[i] = xsun[i];
            } else {
            ...
            for (ij = 0, xp = xx; ij < 4; ij++, xp += 6) {
                 ...
                 * to equator 
                 *********************/
                if (is_true_nodaps && 0 == (iflag & SEFLG_NONUT)) {
                ...
                if (ipli == SE_MOON) {
                    ...
                    if (0 == (iflag & SEFLG_MOSEPH) && !ellipse_is_bary)
                        for (j = 0; j <= 5; j++)
                            xp[j] += xsun[j];
                }
                /*********************
                ...
                if (ipl == SE_SUN && 0 == (iflag & (SEFLG_HELCTR | SEFLG_BARYCTR)))
                    for (j = 0; j <= 5; j++)
                        xp[j] = -xp[j];
                /*********************
                 * light deflection 
                ...
                if ((iflag & SEFLG_SIDEREAL) != 0) {
                    ...
                    if ((swed.sidd.sid_mode & SE_SIDBIT_ECL_T0) != 0) {
                        if (swi_trop_ra2sid_lon(x2000, pldat.xreturn .GetPointer(  6), pldat.xreturn .GetPointer(  18), iflag, ref serr) != OK)
                            return ERR;
                        /* project onto solar system equator */
                    } else if ((swed.sidd.sid_mode & SE_SIDBIT_SSY_PLANE) != 0) {
                        if (swi_trop_ra2sid_lon_sosy(x2000, pldat.xreturn .GetPointer(  6), pldat.xreturn .GetPointer(  18), iflag,ref  serr) != OK)
                            return ERR;
                    } else {
                        /* traditional algorithm */
                        swi_cartpol_sp(pldat.xreturn .GetPointer(  6), pldat.xreturn);
                        pldat.xreturn[0] -= swe_get_ayanamsa(tjd_et) * DEGTORAD;
                        swi_polcart_sp(pldat.xreturn, pldat.xreturn.GetPointer(  6));
                    }
                }
                if ((iflag & SEFLG_XYZ) != 0 && (iflag & SEFLG_EQUATORIAL) != 0) {
                    for (j = 0; j <= 5; j++)
                 ...
                 ************************************************/
                swi_cartpol_sp(pldat.xreturn.GetPointer(  18), pldat.xreturn .GetPointer(  12));
                swi_cartpol_sp(pldat.xreturn.GetPointer( 6), pldat.xreturn);
                /********************** 
                 * radians to degrees *
                 **********************/
        ...
        public Int32 swe_gauquelin_sector(double t_ut, Int32 ipl, string starname, Int32 iflag, Int32 imeth, double[] geopos, double atpress, double attemp, ref double dgsect, ref string serr) {
            ...
            if (imeth == 0 || imeth == 1) {
                ...
                if (imeth == 1)
                    x0[1] = 0;
                dgsect = swe_house_pos(armc, geopos[1], eps + nutlo[1], 'G', x0, ref sdummy);
                return OK;
            }
            /* 
            ...
            retval = swe_rise_trans(t_ut, ipl, starname, epheflag, SE_CALC_RISE | risemeth, geopos, atpress, attemp, ref(tret[0]), ref serr);
            if (retval == ERR) {
                return ERR;
            } else if (retval == -2) {
            ...
            retval = swe_rise_trans(t_ut, ipl, starname, epheflag, SE_CALC_SET | risemeth, geopos, atpress, attemp, ref (tret[1]), ref serr);
            if (retval == ERR) {
                return ERR;
            } else if (retval == -2) {
                set_found = false;
            ...
            if (tret[0] < tret[1] && rise_found == true) {
                ...
                set_found = true;
                retval = swe_rise_trans(t, ipl, starname, epheflag, SE_CALC_SET | risemeth, geopos, atpress, attemp, ref (tret[1]), ref serr);
                if (retval == ERR) {
                    return ERR;
                } else if (retval == -2) {
                    set_found = false;
                ...
                rise_found = true;
                retval = swe_rise_trans(t, ipl, starname, epheflag, SE_CALC_RISE | risemeth, geopos, atpress, attemp, ref (tret[0]), ref serr);
                if (retval == ERR) {
                    return ERR;
                } else if (retval == -2) {
                    rise_found = false;
