...
//                           double *ascmc);
        ...
        int sidereal_houses_ecl_t0(double tjde,
            ...
            swi_coortrf(x.GetPointer(3), x.GetPointer(3), -epst0);
            /* to tjd_et */
            swi_precess(x, sip.t0, 0, J_TO_J2000);
            swi_precess(x, tjde, 0, J2000_TO_J);
            swi_precess(x.GetPointer(3), sip.t0, 0, J_TO_J2000);
            swi_precess(x.GetPointer(3), tjde, 0, J2000_TO_J);
            /* to true equator of tjd_et */
            swi_coortrf(x, x, (eps - nutlo[1]) * DEGTORAD);
            swi_coortrf(x.GetPointer(3), x.GetPointer(3), (eps - nutlo[1]) * DEGTORAD);
            swi_cartpol_sp(x, x);
            x[0] += nutlo[0] * DEGTORAD;
            swi_polcart_sp(x, x);
            swi_coortrf(x, x, -eps * DEGTORAD);
            swi_coortrf(x.GetPointer(3), x.GetPointer(3), -eps * DEGTORAD);
            /* now, we have the moving point precessed to tjd_et.
             * next, we compute the auxiliary epsilon: */
            swi_cross_prod(x, x.GetPointer(3), xnorm);
            rxy = xnorm[0] * xnorm[0] + xnorm[1] * xnorm[1];
            c2 = (rxy + xnorm[2] * xnorm[2]);
            rxyz = Math.Sqrt(c2);
            rxy = Math.Sqrt(rxy);
            epsx = Math.Asin(rxy / rxyz) * RADTODEG;           /* 1a */
            ...
            dvpxe = Math.Acos(swi_dot_prod_unit(x, xvpx)) * RADTODEG;  /* 5 */
            if (tjde < sip.t0)
                dvpxe = -dvpxe;
            for (i = 1; i <= ito; i++)                     /* 6, 7 */
        ...
        int sidereal_houses_ssypl(double tjde,
            ...
            swi_coortrf(x.GetPointer(3), x.GetPointer(3), -SSY_PLANE_INCL);
            swi_cartpol_sp(x, x);
            x[0] += SSY_PLANE_NODE_E2000;
            swi_polcart_sp(x, x);
            /* to equator 2000 */
            swi_coortrf(x, x, -eps2000);
            swi_coortrf(x.GetPointer(3), x.GetPointer(3), -eps2000);
            ...
            switch (hsy) {
                ...
                case 'M': {
                        ...
                        for (i = 1; i <= 12; i++) {
                            j = i + 10;
                            if (j > 12) j -= 12;
                            a3 = swe_degnorm(a3 + 30);
                            x3[0] = a3;
                            x3[1] = 0;
                            swe_cotrans(x3, x3, ekl);
                            hsp.cusp[j] = x3[0];
                        }
                        acmc = swe_difdeg2n(hsp.ac, hsp.mc);
                        if (acmc < 0) {
    ...
    if (Math.Abs(fi) >= 90 - ekl) {  /* within polar circle */
      ...
      goto porphyry;
                         ...
                         */
                        sda = Math.Acos(r) * RADTODEG;	/* semidiurnal arc, measured on equator */
        ...
        public double swe_house_pos(
            ...
            switch (hsys) {
                ...
                case 'W':
                    ...
                    if (hsys == 'W')
                        xp[0] = swe_degnorm(xp[0] + (asc % 30.0));
