...
namespace SweNet
{
    ...
    partial class SwissEph
    {
         ...
         */
        int swi_pleph(double et, int ntarg, int ncent, CPointer<double> rrd, ref string serr) {
            int i, retc;
            ...
            if (ntarg == J_LIB) {
                if (js.eh_ipt[37] > 0) {
                    list[11] = 2;
                    if ((retc = state(et, list, false, pv, pvsun, rrd, ref serr)) != OK)
                        return (retc);
                    for (i = 0; i < 6; ++i)
                        rrd[i] = pv[i + 60];
                    return 0;
            ...
            if (ncent == J_EMB) 	/* EMB needs Earth */
                list[J_EARTH] = 2;
            if ((retc = state(et, list, true, pv, pvsun, rrd, ref serr)) != OK)
                return (retc);
